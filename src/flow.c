...
any doAt(any ex) {
   ...
   NeedNum(ex,car(x));
   NeedNum(ex,cdr(x));
   if (num(car(x) = (any)(num(car(x)) + OneWithoutTag)) < num(cdr(x)))
      return Nil;
   car(x) = Zero;
   return prog(cddr(ex));
}
...
any doFor(any x) {
   ...
   if (!isCell(y = car(x = cdr(x))) || !isCell(cdr(y))) {
      ...
      for (;;) {
         if (isNum(data(c1))) {
            val(f.bnd[0].sym) = (any)(num(val(f.bnd[0].sym)) + OneWithoutTag);
         ...
         if (f.cnt == 2)
            val(f.bnd[1].sym) = (any)(num(val(f.bnd[1].sym)) + OneWithoutTag);
         do {
            if (!isNum(y = car(x))) {
               if (isSym(y))
                  y = val(y);
   ...
   for (;;) {
      if (f.cnt == 2)
         val(f.bnd[1].sym) = (any)(num(val(f.bnd[1].sym)) + 8);
